...
export function useChallengeMode(
  ...
  const getNextRandomTarget = useCallback((completed: string[]) => {
    const remaining = BONE_HOTSPOTS.filter(h => !completed.includes(h.id));
  ...
  const handleChallengeClick = useCallback(
    (clickedId: string) => {
      ...
      if (clickedId === currentTargetId) {
        ...
        if (newCompleted.length === BONE_HOTSPOTS.length) {
  ...
  const currentTargetLabel = useMemo(() => {
    ...
    const hotspot = BONE_HOTSPOTS.find(h => h.id === currentTargetId);
