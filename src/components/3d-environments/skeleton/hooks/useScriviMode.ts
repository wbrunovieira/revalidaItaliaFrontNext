...
import { BONE_HOTSPOTS, BONE_PARTS } from '../data';
...
export function useScriviMode(
  ...
  const getRandomScriviTarget = useCallback((used: string[]) => {
    const available = BONE_HOTSPOTS.filter(h => !used.includes(h.id));
  ...
  const getHotspotCameraPosition = useCallback((hotspotId: string) => {
    ...
    // Skull: y > 155
    if (yPos > 155) {
      return BONE_PARTS.find(p => p.id === 'skull');
    }
    // Ribcage/Spine upper: y > 100
    if (yPos > 100) {
      return BONE_PARTS.find(p => p.id === 'ribcage');
    }
    // Spine/Pelvis: y > 50
    if (yPos > 50) {
      return BONE_PARTS.find(p => p.id === 'spine');
    }
    // Limbs: y <= 50
    return BONE_PARTS.find(p => p.id === 'limbs');
  }, []);
  ...
  const handleScriviSubmit = useCallback(() => {
    ...
    const hotspot = BONE_HOTSPOTS.find(h => h.id === scriviTargetId);
    if (!hotspot) return;
  ...
  const getScriviDiagnosis = useCallback(() => {
    ...
    if (percentage === 100)
      return { emoji: '🏆', title: 'Scrittura Perfetta!', message: 'Conosci tutti i nomi delle ossa!' };
    ...
    if (percentage >= 40)
      return { emoji: '📚', title: 'Devi studiare!', message: 'Ripassa i nomi delle ossa.' };
