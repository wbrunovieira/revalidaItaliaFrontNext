...
export default function LiveSessions({ locale }: LiveSessionsProps) {
  ...
  const displaySessions = currentSessions.map((session: any) => {
    // Check if it's API format by looking for specific fields
    if ('settings' in session && 'host' in session) {
      // API format - map the actual API response structure
      return {
        ...
        instructor: {
          id: session.host.id,
          name: session.host.fullName || session.host.name || 'Host',
        ...
        recordingUrl: session.recordingUrl || (session.recordingAvailable ? '#' : null),
        participantsCount: session.participantCount || 0,
        maxParticipants: session.settings?.maxParticipants || session.maxParticipants || 100,
