...
export default function NavSidebar({
  ...
  const checkBackendConnection = async (isRetry = false): Promise<boolean> => {
    if (isRetry) {
      setIsRetrying(true);
    }
    ...
    try {
      ...
      const hasErrors = responses.some(result => 
  ...
  }, [connectionStatus, retryCount, isHydrated]);
  ...
  return (
    <div className="flex">
      ...
      >
        ...
        {isHydrated && connectionStatus === 'error' && connectionError && (
        ...
        {isHydrated && connectionStatus === 'checking' && (
          <div className="bg-gradient-to-r from-yellow-600 to-yellow-700 border-b border-yellow-500">
            <div className="flex items-center justify-center px-4 py-2">
              <div className="flex items-center gap-3">
                <RefreshCw size={16} className="text-yellow-200 animate-spin" />
        ...
        {isHydrated && connectionStatus === 'online' && retryCount > 0 && (
