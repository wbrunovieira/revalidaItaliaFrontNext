...
export default function StudentAssessmentStatus({ userId, locale }: StudentAssessmentStatusProps) {
  ...
  const fetchAttempts = async (showRefreshing = false) => {
    ...
    try {
      ...
      console.log('StudentAssessmentStatus - API Response:', data);
      console.log('StudentAssessmentStatus - User ID:', userId);
      ...
      console.log('StudentAssessmentStatus - Filtered user open attempts:', userOpenAttempts.length);
  ...
  const getStatusIcon = (status: string, pendingReview: number) => {
    switch (status) {
      case 'IN_PROGRESS':
  ...
  const getActionText = (status: string, pendingReview: number) => {
    switch (status) {
      case 'IN_PROGRESS':
        return t('assessments.actions.continue');
      case 'SUBMITTED':
        if (pendingReview > 0) {
          return 'Responder Novamente';
        }
        return t('assessments.actions.viewResult');
      case 'GRADING':
        return t('assessments.actions.waitReview');
      case 'GRADED':
        return t('assessments.actions.viewResult');
      default:
        return t('assessments.actions.viewDetails');
    }
  };

  const canRetake = (attempt: StudentAttempt) => {
    return attempt.status === 'GRADED' && attempt.passed === false;
  };

  const handleActionClick = (attempt: StudentAttempt) => {
    if (attempt.status === 'SUBMITTED' && attempt.pendingReview > 0) {
      // Redirecionar para a prova para responder novamente
      window.location.href = `/${locale}/courses/course-slug/modules/module-slug/lessons/lesson-id/assessments/${attempt.assessment.id}`;
    } else if (attempt.status === 'GRADED' || (attempt.status === 'SUBMITTED' && attempt.pendingReview === 0)) {
      // Ver resultados
      // TODO: Implementar página de resultados
      console.log('Ver resultados da prova:', attempt.id);
    }
  };
    ...
    return (
      <div className="bg-white/10 rounded-lg p-6 backdrop-blur-sm">
        ...
        <div className="text-center py-12">
          ...
          <p className="text-gray-400">{t('assessments.loading')}</p>
        </div>
      </div>
    );
  }
    ...
    <div className="bg-white/10 rounded-lg p-6 backdrop-blur-sm">
      ...
      {attempts.length === 0 ? (
        <div className="text-center py-12">
          ...
          <p className="text-gray-400 mb-2">{t('assessments.noAssessments')}</p>
        ...
        <div className="space-y-4">
          {attempts.map((attempt) => (
            <div
              key={attempt.id}
              className={`p-4 rounded-lg border-2 ${getStatusColor(attempt.status, attempt.pendingReview)} transition-all hover:border-opacity-60`}
            >
              <div className="flex items-start justify-between mb-3">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <div className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs ${getStatusColor(attempt.status, attempt.pendingReview)}`}>
                      {getStatusIcon(attempt.status, attempt.pendingReview)}
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                    <div>
                      <p className="text-gray-400">{t('assessments.questions')}</p>
                      <p className="text-white font-medium">{attempt.totalQuestions}</p>
                    </div>
                    <div>
                      <p className="text-gray-400">{t('assessments.pending')}</p>
                      <p className={`font-medium ${attempt.pendingReview > 0 ? 'text-yellow-400' : 'text-green-400'}`}>
                        {attempt.pendingReview}
                      </p>
                    </div>
                    <div>
                      <p className="text-gray-400">{t('assessments.approved')}</p>
                      <p className="text-green-400 font-medium">{attempt.reviewedQuestions}</p>
                    </div>
                    {attempt.scorePercentage !== undefined && (
                      <div>
                        <p className="text-gray-400">{t('assessments.score')}</p>
                ...
                <div className="flex items-center gap-4 text-xs text-gray-400">
                  {attempt.submittedAt && (
                    <div className="flex items-center gap-1">
                      <Calendar size={14} />
                      <span>{t('assessments.submitted')}: {formatDate(attempt.submittedAt)}</span>
                    </div>
                  )}
                  {attempt.gradedAt && (
                    <div className="flex items-center gap-1">
                      <CheckCircle size={14} />
                      <span>{t('assessments.graded')}: {formatDate(attempt.gradedAt)}</span>
                    </div>
                  )}
                </div>
