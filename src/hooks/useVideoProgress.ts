...
export function useVideoProgress(
  ...
  useEffect(() => {
    console.log(
      `${LOG_PREFIX} 🚀 Initializing for lesson:`,
      lessonId
    );
    ...
    try {
      ...
      if (savedData) {
        ...
        console.log(
          `${LOG_PREFIX} 📥 Loaded saved progress:`,
          {
            lessonId,
            currentTime: parsedProgress.currentTime,
            percentage: `${parsedProgress.percentage.toFixed(
              2
            )}%`,
            duration: parsedProgress.duration,
            watchedSegments:
              parsedProgress.watchedSegments?.length || 0,
        ...
        console.log(
          `${LOG_PREFIX} 📭 No saved progress found for lesson:`,
          lessonId
        );
      }
    } catch (error) {
      console.error(
        `${LOG_PREFIX} ❌ Error loading progress:`,
        error
      );
    } finally {
      setIsLoading(false);
      ...
      try {
        ...
        if (dataToSave !== lastSavedProgressRef.current) {
          ...
          console.log(`${LOG_PREFIX} 💾 Progress saved:`, {
            ...
            percentage: `${progressToSave.percentage.toFixed(
              2
            )}%`,
          ...
          console.log(
            `${LOG_PREFIX} ⏭️ Skipping save (no changes)`
          );
        }
      } catch (error) {
        console.error(
          `${LOG_PREFIX} ❌ Error saving progress:`,
          error
        );
      }
    },
  ...
  const updateProgress = useCallback(
    (newProgress: VideoProgress) => {
      console.log(
        `${LOG_PREFIX} 📊 Progress update received:`,
        {
          lessonId,
          currentTime: newProgress.currentTime,
          percentage: `${newProgress.percentage.toFixed(
            2
          )}%`,
      ...
      debounceTimerRef.current = setTimeout(() => {
        console.log(
          `${LOG_PREFIX} ⏱️ Debounce timer fired, saving progress...`
        );
        saveProgress(newProgress);

        // Send to heartbeat service for backend sync
        console.log(
          `${LOG_PREFIX} 💓 Sending to heartbeat service`
        );
        heartbeatService.current.enqueue(
          lessonId,
          newProgress,
          courseId,
          moduleId
        ...
        if (
          newProgress.percentage >= 95 &&
          (!progress || progress.percentage < 95)
        ) {
          console.log(
            `${LOG_PREFIX} 🎉 Video completed! (${newProgress.percentage.toFixed(
              2
            )}% watched)`
          );
        }
  ...
  const clearProgress = useCallback(() => {
    console.log(
      `${LOG_PREFIX} 🗑️ Clearing progress for lesson:`,
      lessonId
    );
    ...
    try {
      ...
      console.log(
        `${LOG_PREFIX} ✅ Progress cleared successfully`
      );
    } catch (error) {
      console.error(
        `${LOG_PREFIX} ❌ Error clearing progress:`,
        error
      );
    }
  }, [lessonId, storageKey]);
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (debounceTimerRef.current) {
        console.log(
          `${LOG_PREFIX} 🔄 Component unmounting, forcing save...`
        );
        clearTimeout(debounceTimerRef.current);
        ...
        if (progress) {
          ...
          console.log(
            `${LOG_PREFIX} 💓 Forcing heartbeat flush before unmount`
          );
          currentHeartbeatService.flush();
        }
      }
    };
  ...
  useEffect(() => {
    try {
      ...
      console.error(
        `${LOG_PREFIX} ❌ Error checking storage:`,
        error
      );
    }
  }, [lessonId]);
