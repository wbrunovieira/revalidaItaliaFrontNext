// Panda Player service with fallback strategies
...
import { detectBrowserFeatures, getBrowserInfo } from '@/utils/browser-detection';
...
export class PandaPlayerService {
  ...
  static getInstance(): PandaPlayerService {
    if (!PandaPlayerService.instance) {
      PandaPlayerService.instance = new PandaPlayerService();
    }
  ...
  async loadPandaScript(): Promise<void> {
    ...
    this.loadPromise = new Promise((resolve, reject) => {
      ...
      script.src = 'https://player.tv.pandavideo.com.br/api.v2.js';
      script.async = true;
      ...
      script.onerror = () => {
        this.scriptLoading = false;
        reject(new Error('Failed to load Panda Player script'));
      };
  ...
  async createPlayer(
    ...
    console.log(`Using player strategy: ${this.currentStrategy.name}`);

    switch (this.currentStrategy.name) {
      case 'api-v2':
        return this.createAPIv2Player(elementId, options);
      case 'iframe-enhanced':
        return this.createEnhancedIframePlayer(elementId, options);
      case 'iframe-basic':
        return this.createBasicIframePlayer(elementId, options);
      default:
        return null;
    }
  }
  ...
  private async createAPIv2Player(
    ...
    try {
      ...
      if (!window.PandaPlayer) {
        console.warn('PandaPlayer not available, falling back');
        return this.createEnhancedIframePlayer(elementId, options);
      }
      ...
      return new Promise((resolve) => {
        window.pandascripttag!.push(() => {
          try {
            console.log('Creating PandaPlayer with options:', options);
            ...
            console.log('Creating player in container:', elementId);
            ...
            if (pullzone) {
              ...
              console.log('Adding pullzone to pandaOptions:', pullzone);
            }

            // Add player configurations if provided
            if (options.playerConfigs) {
              // Map our config to Panda's expected format
              if (options.playerConfigs.autoplay !== undefined) {
                pandaOptions.autoplay = options.playerConfigs.autoplay;
              }
              if (options.playerConfigs.muted !== undefined) {
                pandaOptions.muted = options.playerConfigs.muted;
              }
              if (options.playerConfigs.startTime !== undefined) {
                pandaOptions.startTime = options.playerConfigs.startTime;
              }
              if (options.playerConfigs.saveProgress !== undefined) {
                pandaOptions.saveProgress = options.playerConfigs.saveProgress;
              }
              if (options.playerConfigs.smartAutoplay !== undefined) {
                pandaOptions.smartAutoplay = options.playerConfigs.smartAutoplay;
              }
              if (options.playerConfigs.controls !== undefined) {
                pandaOptions.controls = options.playerConfigs.controls;
              }
              if (options.playerConfigs.color !== undefined) {
                pandaOptions.color = options.playerConfigs.color;
              }
              if (options.playerConfigs.controlsColor !== undefined) {
                pandaOptions.controlsColor = options.playerConfigs.controlsColor;
              }
            }
            
            console.log('Panda options:', pandaOptions);
            console.log('Library ID being passed:', options.library_id);
            ...
            try {
              ...
              player = new window.PandaPlayer!(elementId, pandaOptions);
              console.log('PandaPlayer instance created with video:', options.video_id, 'library:', options.library_id);
              
              // Check available methods for debugging
              if (process.env.NODE_ENV === 'development') {
                console.log('Player methods:', Object.getOwnPropertyNames(Object.getPrototypeOf(player)));
              }
              
              // Set up event listeners IMMEDIATELY after creation
              if (player.onEvent && typeof player.onEvent === 'function') {
                console.log('Setting up onEvent listener');
                player.onEvent((event: any) => {
                  console.log('Panda Player Event:', event);
                  
                  // Handle ready event
                  if (event.message === 'panda_ready' || event.message === 'ready') {
                    console.log('Player is ready via event!');
                    isReady = true;
                    if (options.onReady) {
                      options.onReady();
                    }
                  ...
                  // Handle error event
                  if (event.message === 'panda_error' || event.message === 'error') {
                  ...
                  if (event.message) {
                    console.log(`Player event: ${event.message}`, event);
                  }
                });
              } else {
                console.warn('Player.onEvent is not available');
              }
              
              // Set up ready callback directly on player if available
              if (player.onReady && typeof player.onReady === 'function' && options.onReady) {
                player.onReady(options.onReady);
              }
              
              // Set up error callback directly on player if available
              if (player.onError && typeof player.onError === 'function' && options.onError) {
              ...
              const checkPlayerReady = () => {
                if (!isReady) {
                  ...
                  try {
                    ...
                    if (iframe) {
                      console.log('Player iframe found:', iframe.src);
                      ...
                      if (currentSrc.includes(`player-${libraryId}`) && pullzone) {
                        const correctSrc = currentSrc.replace(`player-${libraryId}`, `player-${pullzone}`);
                        console.log('Fixing iframe URL from:', currentSrc);
                      ...
                      if (!iframe.dataset.loadChecked) {
                        ...
                        if (iframe.contentWindow) {
                          console.log('Iframe already loaded, triggering ready');
                          if (!isReady && options.onReady) {
                            isReady = true;
                            options.onReady();
                          }
                      ...
                      try {
                        ...
                        console.log('Iframe has content:', !!iframeDoc);
                      } catch (e) {
                        console.log('Cannot access iframe content (CORS) - this is normal for cross-origin iframes');
                      }
                    } else {
                      console.warn('No iframe found in player container, retrying...');
                      setTimeout(checkPlayerReady, 500);
                    }
                  } catch (checkError) {
                    console.error('Player functionality check failed:', checkError);
                  }
                }
              };
            ...
            console.error('Error creating PandaPlayer:', error);
            resolve(this.createEnhancedIframePlayer(elementId, options));
          }
        });
      });
    } catch (error) {
      console.error('Failed to load API v2, falling back:', error);
      return this.createEnhancedIframePlayer(elementId, options);
    }
  }
  ...
  private async createEnhancedIframePlayer(
    ...
    console.log('Enhanced iframe - pullzone:', pullzone, 'videoId:', videoId);
    
    // Build URL with parameters
    const params = new URLSearchParams();
    params.set('controls', '1'); // Force controls to show
    if (options.playerConfigs?.autoplay) params.set('autoplay', '1');
    if (options.playerConfigs?.muted) params.set('muted', '1');
    if (options.playerConfigs?.startTime) params.set('t', options.playerConfigs.startTime.toString());
    ...
    return this.createEnhancedPlayerInstance(iframe, options);
  }
  ...
  private createEnhancedPlayerInstance(
    ...
    let eventCallbacks: ((event: PandaPlayerEvent) => void)[] = [];
    let isPlaying = false;
    let currentTime = 0;
    let duration = 0;
    ...
    return {
      ...
      togglePlay: () => sendCommand(isPlaying ? 'pause' : 'play'),
      getCurrentTime: () => currentTime,
      getDuration: () => duration,
      setCurrentTime: (time: number) => sendCommand('seek', { time }),
      setVolume: (volume: number) => sendCommand('setVolume', { volume }),
      getVolume: () => 1, // Default
      isPaused: () => !isPlaying,
      isMuted: () => false, // Default
      isFullscreen: () => document.fullscreenElement === iframe,
      toggleFullscreen: () => {
      ...
      setSpeed: (speed: number) => sendCommand('setSpeed', { speed }),
      onEvent: (callback: (event: PandaPlayerEvent) => void) => {
  ...
  private async createBasicIframePlayer(
    ...
    console.log('Basic iframe - pullzone:', pullzone, 'videoId:', videoId);
  ...
  isFeatureSupported(feature: string): boolean {
    return this.currentStrategy?.features.includes(feature) || false;
  }
}