// Video Progress Heartbeat Service with local cache and fallbacks
...
export class VideoProgressHeartbeat {
  ...
  constructor(config?: HeartbeatConfig) {
    ...
    this.API_ENDPOINT = config?.apiEndpoint || '/api/v1/learning-progress/batch';
  ...
  // Singleton pattern
  static getInstance(config?: HeartbeatConfig): VideoProgressHeartbeat {
    if (!VideoProgressHeartbeat.instance) {
      VideoProgressHeartbeat.instance = new VideoProgressHeartbeat(config);
    }
  ...
  private setupEventListeners(): void {
    ...
    window.addEventListener('online', () => {
      console.log(`${LOG_PREFIX} 🌐 Network online - triggering flush`);
      this.isOnline = true;
      this.flush(); // Flush immediately when coming online
    });
    
    window.addEventListener('offline', () => {
      console.log(`${LOG_PREFIX} 📵 Network offline - pausing heartbeat`);
      this.isOnline = false;
    });
    
    // Save queue before page unload
    window.addEventListener('beforeunload', () => {
      console.log(`${LOG_PREFIX} 🔄 Page unloading - saving queue to cache`);
  ...
  private loadQueueFromCache(): void {
    try {
      ...
      if (cached) {
        const updates = JSON.parse(cached) as ProgressUpdate[];
        updates.forEach(update => {
          this.updateQueue.set(update.lessonId, update);
        });
        
        console.log(`${LOG_PREFIX} 📥 Loaded ${updates.length} updates from cache:`, 
          updates.map(u => ({
            lessonId: u.lessonId,
        ...
        console.log(`${LOG_PREFIX} 📭 No cached updates found`);
      }
    } catch (error) {
      console.error(`${LOG_PREFIX} ❌ Error loading cache:`, error);
    }
  }
  ...
  private saveQueueToCache(): void {
    try {
      ...
      if (updates.length > 0) {
        localStorage.setItem(this.CACHE_KEY, JSON.stringify(updates));
        console.log(`${LOG_PREFIX} 💾 Saved ${updates.length} updates to cache`);
      } else {
        localStorage.removeItem(this.CACHE_KEY);
        console.log(`${LOG_PREFIX} 🧹 Cleared cache (queue empty)`);
      }
    } catch (error) {
      console.error(`${LOG_PREFIX} ❌ Error saving to cache:`, error);
    }
  }
  
  // Start heartbeat timer
  private startHeartbeat(): void {
    console.log(`${LOG_PREFIX} ❤️ Starting heartbeat with ${this.FLUSH_INTERVAL}ms interval`);
    
    this.flushInterval = setInterval(() => {
      console.log(`${LOG_PREFIX} ⏰ Heartbeat tick - queue size: ${this.updateQueue.size}`);
      this.flush();
    }, this.FLUSH_INTERVAL);
  }
  ...
  public async flush(): Promise<void> {
    if (!this.isOnline) {
      console.log(`${LOG_PREFIX} 📵 Skip flush - offline (${this.updateQueue.size} updates pending)`);
      return;
    }
    
    if (this.isFlushing) {
      console.log(`${LOG_PREFIX} ⏳ Skip flush - already in progress`);
      return;
    }
    
    if (this.updateQueue.size === 0) {
      console.log(`${LOG_PREFIX} 📭 Skip flush - queue empty`);
      return;
    }
    ...
    console.log(`${LOG_PREFIX} 🚀 Starting flush of ${updates.length} updates:`,
      updates.map(u => ({
        lessonId: u.lessonId,
        percentage: u.progress.percentage.toFixed(2) + '%',
        attempts: u.attempts
    ...
    try {
      ...
      console.log(`${LOG_PREFIX} ✅ Flush successful - cleared ${updates.length} updates`);
      this.saveQueueToCache();
  ...
  private async sendToBackend(updates: ProgressUpdate[]): Promise<void> {
  ...
  private handleFlushError(updates: ProgressUpdate[]): void {
    updates.forEach(update => {
      update.attempts += 1;
      
      if (update.attempts >= this.MAX_RETRY_ATTEMPTS) {
        console.log(`${LOG_PREFIX} 🚫 Max attempts reached, removing update:`, {
          lessonId: update.lessonId,
          attempts: update.attempts
        });
        this.updateQueue.delete(update.lessonId);
      } else {
        console.log(`${LOG_PREFIX} 🔄 Incrementing retry attempt:`, {
          lessonId: update.lessonId,
          attempts: update.attempts
        });
        this.updateQueue.set(update.lessonId, update);
  ...
  public getStatus(silent: boolean = false): {
    ...
    const updates = Array.from(this.updateQueue.values());
    const oldestUpdate = updates.length > 0 
    ...
    if (!silent) {
      console.log(`${LOG_PREFIX} 📊 Status:`, {
        ...status,
        oldestUpdateAge: oldestUpdate ? `${Math.round((Date.now() - oldestUpdate) / 1000)}s ago` : 'N/A'
      });
    }
  ...
  public clearQueue(): void {
    ...
    this.saveQueueToCache();
    console.log(`${LOG_PREFIX} 🧹 Queue cleared (removed ${size} updates)`);
  }
}

// Export singleton instance getter
export const getHeartbeatService = (config?: HeartbeatConfig) => 
